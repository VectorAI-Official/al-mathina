...
document.addEventListener('DOMContentLoaded', function () {
    loadCategories();
    loadProducts();
    loadInventoryItems();  // Load inventory for linking
    setupEventListeners();
});
...
function filterProducts() {
    ...
    // Apply search filter (searches only product names and item_id, NOT categories)
    if (searchTerm) {
        filtered = filtered.filter(product => {
            const productName = (product.product_name || product.name || '').toLowerCase();
            const productNameTa = (product.product_name_ta || '').toLowerCase();
            const itemId = (product.item_id || '').toLowerCase();

            return productName.includes(searchTerm) ||
                productNameTa.includes(searchTerm) ||
...
async function openCreateModal() {
    ...
    // Show Initial Stock input, Hide Inventory Link
    document.getElementById('individualStockMode').style.display = 'block';
    document.getElementById('inventoryLinkedMode').style.display = 'none';
    document.getElementById('productStock').required = true;
    document.querySelector('label[for="productStock"]').textContent = "Initial Stock Quantity *";
    document.querySelector('label[for="productStock"] + .form-hint').textContent = "Initial stock for the new inventory item";


    // Populate searchable inventory dropdown
    populateInventorySearchDropdown();
...
function editProduct(productId) {
    ...
    document.getElementById('productActive').checked = (product.active !== false); // Default to true if undefined

    // EDIT MODE: Hide Stock Input, Show Inventory Link
    document.getElementById('individualStockMode').style.display = 'none';
    document.getElementById('inventoryLinkedMode').style.display = 'block';
    document.getElementById('productStock').required = false;

    // Populate searchable inventory dropdown and set selection
    populateInventorySearchDropdown(); // Ensure list is populated
    setInventorySelection(product.inventory_id);
...
async function handleProductSubmit(e) {
    ...
    try {
        ...
        // Add inventory_id if selected (from hidden input in searchable dropdown)
        const inventoryId = document.getElementById('selectedInventoryId')?.value;

        if (!currentProductId) {
            // CREATE MODE: Send Stock for Initial Inventory Creation
            productData.stock = parseInt(document.getElementById('productStock').value) || 0;
            // inventory_id is auto-generated by backend
            console.log('   📦 New Product: Sending Initial Stock:', productData.stock);
        } else {
            // EDIT MODE: Send Inventory ID (Stock ignored)
            if (inventoryId) {
                productData.inventory_id = inventoryId;
                console.log('   📦 Edit Product: Linking to Inventory ID:', inventoryId);
            } else {
                // Fallback: If no inventory ID (shouldn't happen in strict mode), ensure we don't unlink
                // But wait, user wanted "Pure Inventory". So linking is mandatory.
                // If inventoryId is empty, it means they cleared it?
                // I'll assume they selected something if required.
                console.warn('   ⚠️ Edit Product: No Inventory ID selected?');
                // Maybe keep old one? Backend doesn't require it if not present.
                // But if they explicitly cleared it, we might want to error?
                // For now, let's send what we have.
                productData.inventory_id = inventoryId;
            }
            // Do NOT send stock for updates
...
async function loadInventoryItems() {
    try {
        ...
        // Populate inventory dropdown for search
        populateInventorySearchDropdown();
    } catch (error) {
...
// Populate searchable inventory dropdown
function populateInventorySearchDropdown() {
    const dropdown = document.getElementById('inventoryDropdown');
    if (!dropdown) return;

    if (inventoryItems.length === 0) {
        dropdown.innerHTML = '<div class="dropdown-item no-results">No inventory items found</div>';
        return;
    }

    // Clear and populate with all items initially
    dropdown.innerHTML = '';
    inventoryItems.forEach(item => {
        const div = document.createElement('div');
        div.className = 'dropdown-item';
        div.dataset.inventoryId = item.inventory_id;
        div.innerHTML = `
            <span class="dropdown-item-name">${item.inventory_name}</span>
            <span class="dropdown-item-stock">${item.stock_quantity} ${item.unit || 'units'}</span>
        `;
...
// Filter inventory dropdown based on search input
function filterInventoryDropdown() {
    ...
    if (filteredItems.length === 0) {
        dropdown.innerHTML = '<div class="dropdown-item no-results">No matching inventory items</div>';
        return;
    }
    ...
    filteredItems.forEach(item => {
        ...
        const highlightedName = searchTerm
            ? name.replace(new RegExp(`(${searchTerm})`, 'gi'), '<strong>$1</strong>')
            : name;
...
function showInventoryDropdown() {
    const dropdown = document.getElementById('inventoryDropdown');
    if (dropdown) {
        dropdown.style.display = 'block';
    }
...
function selectInventoryItem(item) {
    const searchInput = document.getElementById('inventorySearch');
    const hiddenInput = document.getElementById('selectedInventoryId');
    const dropdown = document.getElementById('inventoryDropdown');

    // Set values
    searchInput.value = `${item.inventory_name} (${item.stock_quantity} ${item.unit || 'units'})`;
    hiddenInput.value = item.inventory_id;

    // Hide dropdown
    dropdown.style.display = 'none';

    // Highlight selected in dropdown
    dropdown.querySelectorAll('.dropdown-item').forEach(div => {
        div.classList.remove('selected');
        if (div.dataset.inventoryId === item.inventory_id) {
            div.classList.add('selected');
        }
    });
}

// switchStockMode REMOVED - Inventory Mode Enforced
...
async function setInventorySelection(inventoryId) {
    ...
    if (!inventoryId) {
        console.log('   ℹ️ No inventory ID provided');
        return;
    }

    // Ensure inventory mode is visible (it should be for Edit)
    document.getElementById('inventoryLinkedMode').style.display = 'block';
